...
Future<dynamic> showCustomAttendanceDialog(BuildContext context, int employeeID){
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                CustomDatePicker(dateController: birthDateController, hint: "التاريخ",),
              ],
            ),
          ),
        ),
        actions: [
          BlocConsumer<HumanResourcesCubit, HumanResourcesState>(
            listener: (context, state){
              if(state is EstablishAttendanceSuccessState){
                showToast("تم ادراج حضور للموظف بنجاح", ToastState.SUCCESS);
                navigateAndFinish(context, const DrawerLayout());
              }else if(state is EstablishAttendanceFailureState){
              ...
              return CustomButton(
                function: (){
                  if(birthDateController != null){
                    HumanResourcesCubit.get(context).establishAttendance(
